...
void init_rain() {
    for (int i = 0; i < MAX_RAIN; i++) {
        ...
        raindrops[i].speed = (rand() % 5 + 5) / 100.0f;
    }
}
...
void draw_lightfog(){
    GLfloat fogColor[] = {0.4f, 0.4f, 0.4f, 1.0f};
...
void dummy_keyboard(unsigned char key, int x, int y) {

}

void dummy_keyboard_up(unsigned char key, int x, int y) {
...
void display()
{
	...
	glutSwapBuffers();
	if(needhelp == 1){
        glutKeyboardFunc(dummy_keyboard);
        glutKeyboardUpFunc(dummy_keyboard_up);
        glutMouseFunc(idle);
        glutMotionFunc(idle);
...
void key_handler(unsigned char key, int x, int y)
{
    ...
    case '-':
        ...
        break;
	case 'h':
		if (light_diffuse[0] == 1.0f && light_diffuse[1] == 1.0f && light_diffuse[2] == 1.0f) {
        // White -> Sunset
        light_diffuse[0] = 0.9f;
        light_diffuse[1] = 0.3f;
        light_diffuse[2] = 0.1f;
		} else if (light_diffuse[0] == 0.9f && light_diffuse[1] == 0.3f && light_diffuse[2] == 0.1f) {
        // Sunset -> Frosty
...
void initialize()
{
    ...
    texture_id = load_texture("assets/textures/Trench-wall.jpg");
    sky_texture_id = load_texture("assets/textures/sky.jpg");
    ground_texture_id = load_texture("assets/textures/ground.jpg");
    help_texture_id = load_texture("assets/textures/help.png");
    glClearDepth(1.0);
...
{
    load_model("assets/models/grafika.obj", &model);
    load_model("assets/models/ground.obj", &ground);
...
void draw_rain() {
    ...
    for (int i = 0; i < MAX_RAIN; i++) {
        ...
        glVertex3f(raindrops[i].x, raindrops[i].y - 0.2f, raindrops[i].z);
    }
    glEnd();
    glEnable(GL_LIGHTING);
}
